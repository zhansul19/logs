...
async def get_dossie_log_entries(tag: str,
    ...
    if tag == "username":
        log_entries = db.query(DossieLog).filter(DossieLog.user_name == value).filter(DossieLog.action.like('Поиск%'))
        logging.info(f"{value}", extra={'user': current_user,
                                        'table': 'dossie_log',
                                        'action': 'поиск пользователя по user_name'})
    if tag == "username_partial":
        log_entries = db.query(DossieLog).filter(cast(DossieLog.user_name, String).contains(value)).filter(DossieLog.action.like('Поиск%'))
        logging.info(f"{value}", extra={'user': current_user,
                                        'table': 'dossie_log',
                                        'action': 'поиск пользователя по user_name'})
    elif tag == "fullname":
        ...
        log_entries = db.query(DossieLog).filter(func.concat(DossieLog.lname, ' ', DossieLog.fname, ' ', DossieLog.mname)
                                                 .contains(value)).filter(DossieLog.action.like('Поиск%'))
        logging.info(f"{value}", extra={'user': current_user,
                                        'table': 'dossie_log',
        ...
        log_entries = db.query(DossieLog).filter(DossieLog.action.like(f'%{value}%')).filter(DossieLog.action.like('Поиск%'))
        logging.info(f"{value}", extra={'user': current_user,
                                        'table': 'dossie_log',
                                        'action': 'поиск по иин'})
...
async def get_dossie_fullname_log_entries(lname: str = Query(None),
    ...
    log_entries = db.query(DossieLog).filter(combined_filter).filter(DossieLog.action.like('Поиск%'))
    # Filter by date range
    if start_date:
        log_entries = log_entries.filter(DossieLog.log_time >= start_date)
    if end_date:
