...
async def get_users_log_entries_by_username(tag: str, value: str, current_user: str = Depends(get_current_user), db: Session = Depends(get_db)):
    if tag == "username":
        log_entries = db.query(UsersLog.username,Cascade.name,UsersLog.message,UsersLog.debug_level,UsersLog.time).join(Cascade,UsersLog.username == Cascade.email).filter(UsersLog.username==value).all()
        logging.info(f"{value}",extra={'user': current_user, 'table': 'users_log_cascade', 'action': 'поиск пользователя по username'})
    elif tag == "fullname":
        log_entries = db.query(UsersLog.username,Cascade.name,UsersLog.message,UsersLog.debug_level,UsersLog.time).join(Cascade,UsersLog.username == Cascade.email).filter(cast(Cascade.name, String).contains(value)).all()
        logging.info(f"{value}", extra={'user': current_user, 'table': 'users_log_cascade', 'action': 'поиск пользователя по фио'})
    elif tag == "message":
        log_entries = db.query(UsersLog.username,Cascade.name,UsersLog.message,UsersLog.debug_level,UsersLog.time).join(Cascade,UsersLog.username == Cascade.email).filter(cast(UsersLog.message, String).contains(value)).all()
