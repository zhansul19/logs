...
from sqlalchemy import String, cast, TIMESTAMP
import re
...
async def get_users_log_entries_by_username(tag: str,
    ...
    log_entries = db.query(UsersLog.username, Cascade.name, UsersLog.message, UsersLog.debug_level, UsersLog.time).join(Cascade, UsersLog.username == Cascade.email)
    if tag == "username":
